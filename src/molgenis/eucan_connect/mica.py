...
class Mica:
    """
    This class is responsible for retrieving data from the source catalogues stored at a Mica server and convert it
    to the EUCAN-Connect Catalogue data model.
    """
    ...
    def mica_data(self, catalogue: Catalogue) -> CatalogueData:
        ...
        The study data category contains, besides study information, also person, data collection events and population data.
        For the conversion to the EUCAN-Connect catalogue only data from this category is selected as the other three data categories
        ...
        if len(mica_studies) != len(mica_study_data):
            # raise EucanError(f"Number of studies with data ({len(mica_study_data)}) does not equal the total number of studies ({len(mica_studies)})")
            warning = EucanWarning(
                f"Number of studies with data ({len(mica_study_data)}) does not equal the total number of studies ({len(mica_studies)})"
            )
    ...
    def get_mica_study_ids(self, start: int = 0, batch_size: int = 100) -> List:
        ...
        The REST API Client that is used here returns not only the IDs, but also study information.
        It turns however out that this is 'summary' information and it does not contain all necessary data
        for the EUCAN-Connect Catalogue. Therefore this API is only used for retrieving a list with study IDs.
        Depending on the value of param amount only the total amount of studies or the real study information is returned
            @param start: study number (row) to start from, default is 0
            @param batch_size: number of studies to be returned if != totalCount, a loop should be created
        """
        studies = []
    ...
    def get_mica_study_data(self, mica_studies: List):
        """
        Get the data of all Mica studies from a catalogue by using the Mica REST API Client. It returns the full study data
            ...
            for pop in study["populations"]:
                try:
                            ...
                            if "dataSources" in dce_content.keys():
                                for source in dce_content.get("dataSources"):
                                    ...
                                    if source not in self.ref_data.all_refs(
                                        ...
                                        self.printer.print(
                                            f"A new DataSource: {source} will be added to the EUCAN-Connect Catalogue"
                                        )
                            if "bioSamples" in dce_content.keys():
                                for sample in dce_content.get("bioSamples"):
                                    ...
                                    if sample not in self.ref_data.all_refs(
                                        ...
                                        self.printer.print(
                                            f"A new bioSample: {sample} will be added to the EUCAN-Connect Catalogue"
                                        )
                            if "administrativeDatabases" in dce_content.keys():
                                for db_type in dce_content.get(
                                    ...
                                    if db_type not in self.ref_data.all_refs(
                                        ...
                                        self.printer.print(
                                            f"A new database type: {db_type} will be added to the EUCAN-Connect Catalogue"
                                        )
                    ...
                    if e.args[0] == "dataCollectionEvents":
                        ...
                        raise EucanError(
                            f" KeyError in function _to_events: {e.args[0]} does not exist"
                        )
                        ...
                        if not any(
                            ...
                            try:
                                ...
                                if e.args[0] == ["firstName"]:
                                    warning = EucanWarning(
                                        f"Person {member['id']} of study {study['id']} has no first name"
                                    )
                                    self.printer.print_warning(warning)
                                    self.warnings.append(warning)
                            except Exception as e:
                                raise EucanError(
                                    f" An error occurred in function _to_persons defining first name: {e}"
                                )
                            ...
                            try:
                                ...
                                if not person["country"]:
                                    warning = EucanWarning(
                                        f"Country {member['institution']['address']['country']['iso']} not found in the IsoCountryData"
                                    )
                                    self.printer.print_warning(warning)
                                    self.warnings.append(warning)
                            except KeyError as e:
                                if e.args[0] in ["institution", "address", "country"]:
                                    ...
                                    raise EucanError(
                                        f" KeyError in function _to_persons defining country: {e.args[0]} does not exist"
                                    )
                            except Exception as e:
                                raise EucanError(
                                    f" An error occurred in function _to_persons defining country: {e}"
                                )
            ...
            for pop in study["populations"]:
                ...
                if "content" in pop:
                    ...
                    if "selectionCriteria" in pop_content.keys():
                        ...
                        for criterium in selection_criteria.keys():
                            try:
                                ...
                                # elif type(pop_content["selectionCriteria"][criterium]) is str:
                                #     print(criterium, pop_content["selectionCriteria"][criterium])
                                #     if len(pop_content["selectionCriteria"][criterium]) > 0: # \
                                #          #   or pop_content["selectionCriteria"][criterium] != "None":
                                #         population["selection_criteria"].append(selection_criteria[criterium])
                                else:
                                    warning = EucanWarning(
                                        f" Undefined pop_content selection criteria type {pop_content['selectionCriteria'][criterium]} found for {criterium}"
                                    )
                                    self.printer.print_warning(warning)
                                    self.warnings.append(warning)
                    ...
                    if "recruitment" in pop_content.keys():
                        ...
                        try:
                            if (
                                not "recruitment_sources_supplement"
                                in population.keys()
                            ):
                        ...
                        try:
                            ...
                            for source in pop_content["recruitment"]["dataSources"]:
                                ...
                                if source not in self.ref_data.all_refs(
                                    ...
                                    self.printer.print(
                                        f"A new recruitment source {source} will be added to the EUCAN-Connect Catalogue"
                                    )
                        except KeyError as e:
                            if e.args[0] != "dataSources":
                                raise EucanError(
                                    f" An unexpected KeyError occurred in function _to_populations: {e}"
                                )
                        except Exception as e:
                            raise EucanError(
                    ...
                    if "numberOfParticipants" in pop_content.keys():
                        try:
                            ...
                            if str(e) != "number":
                                raise EucanError(
                                    f" An error occurred in defining number_of_participants in function _to_populations: {e}"
                                )
                        try:
                            ...
                            if str(e) != "'number'":
                                raise EucanError(
                                    f" An error occurred in defining participant_with_biosamples in function _to_populations: {e}"
                                )
                        try:
                            population[
                            ...
                            if str(e) != "'info'":
                                raise EucanError(
                                    f" An error occurred in defining number_of_participants_supplement in function _to_populations: {e}"
                                )
                    else:
                        warning = EucanWarning(
                            f"Population {population['id']} has no number of participants information"
                        )
                        self.printer.print_warning(warning)
                        self.warnings.append(warning)
                else:
                    warning = EucanWarning(
                        f"Population {population['id']} has no selection criteria and recruitment sources"
                    )
                    self.printer.print_warning(warning)
                    self.warnings.append(warning)
        ...
        for row in study_data:
            ...
            if "content" in row.keys():
                ...
                try:
                    ...
                    pass
                # TODO: access_possiblity and contact_procedures need to be defined in another way in the EUCAN-Connect data model in my opinion
                # TODO: Recap has a list with categories which can be accessed
                # TODO: Maelstrom has a variable yes/no for data, biosamples etc
                # TODO: access contact person and procedure not available in RECAP
                # TODO: Maelstrom does has this info, but then it should become a concatenated string (?)
                ...
                # TODO: study_timeline is not an attribute in Mica (it's an graph based on data collection events data)
