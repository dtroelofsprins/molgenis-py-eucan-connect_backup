...
class EucanSession(ExtendedSession):
    ...
    def get_catalogues(self, codes: List[str] = None) -> List[Catalogue]:
        """
        Retrieves a list of Source catalogues objects from the source catalogues table. Will return
        all source catalogues or some source catalogues if 'codes' is specified.
    ...
    def get_reference_data(self) -> RefData:
        """
        Retrieves the current data from the reference entities in the EUCAN-Connect Catalogue
    ...
    # def get_eucan_connect_data(self, catalogue: Catalogue) -> EucanData:
    #     """
    #     Gets the IDs of the rows in the four tables that belong to a single source catalogue from the EUCAN-Connect tables.
    #     Filters the rows based on the source_catalogue field.
    #
    #     :param Catalogue catalogue: the source catalogue to get the EUCAN-Connect data for
    #     :return: an IdData object
    #     """
    #
    #     tables = dict()
    ...
    #                 id_, batch_size=10000, q=f"source_catalogue=={catalogue.code}"
    #             ),
    #         )
    #
    #     return EucanData.from_dict(node=node, source=Source.PUBLISHED, tables=tables)

    # def get_catalogue(self, code: str) -> Catalogue:
    #     """
    #     Retrieves a single source catalogue object from the source catalogues table.
    #     :param code: source catalogue to get by code
    #     :return: Catalogue object
    #     """
    #     catalogues = self.get(self.CATALOGUES_TABLE, q=f"id=={code}")
    #     self._validate_codes([code], catalogues)
    #     return self._to_catalogues(catalogues)[0]
