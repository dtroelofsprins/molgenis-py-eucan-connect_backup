...
class Importer:
    ...
    def _import_catalogue_data(self, catalogue_data: CatalogueData):
        ...
        1. First all data from the source catalogue is removed from the EUCAN-Connect Catalogue
        ...
        for table in reversed(catalogue_data.import_order):
            self.printer.print(
                f"Deleting {len(table.rows)} rows in {table.type.base_id}"
            )
            try:
    ...
    def _delete_rows(self, table: Table, catalogue: Catalogue):
        """
        Deletes all rows from a EUCAN-Connect Catalogue table from source catalogue of which data
        is imported.
        ...
        """
        # Compare the ids from the source catalogue and production to see what data are deleted
        ...
        for id_ in deleted_ids:
            warning = EucanWarning(
                f"This {catalogue.description} {table.type} ID {id_} is not in the source catalogue anymore."
            )
            self.printer.print_warning(warning)
            self.warnings.append(warning)

        # Actually delete the existing rows of the source catalogue in the EUCAN-Connect Catalogue
        if eucan_ids:
            self.printer.print(
