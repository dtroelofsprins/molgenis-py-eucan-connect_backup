...
from molgenis.eucan_connect.model import Catalogue, CatalogueData, RefData
from molgenis.eucan_connect.printer import Printer


class Eucan:
    """
    Main class for importing data from source catalogues into the EUCAN-Connect Catalogue.
    """

    def __init__(self, session: EucanSession):
        """
        :param EucanSession session: an authenticated session with an EUCAN-Connect Catalogue
        """
    ...
    def import_catalogues(self, catalogues: List[Catalogue]) -> ErrorReport:
        """
        Imports data from the provided source catalogue(s) into the tables in the EUCAN-Connect
        catalogue.
    ...
    def _add_new_ref_data(
        ...
        """
        self.warnings = []
        self.printer.print_sub_header(f"📤 Importing new reference data")
        self.printer.indent()
    ...
    def _import_catalogue_data(
        ...
        1. First all data from the source catalogue are removed from the EUCAN-Connect Catalogue
